...
LOGGER = logging.getLogger("samples.lucas_auditor_ollama.client")
LUCAS_TOOL_NAME = "lucas_county_auditor_search"


def local_src_paths(root: Path) -> list[Path]:
    # The Lucas auditor tool ships in the shared iasWorld platform package plus
...
def lucas_server_params(root: Path, log_level: str) -> Any:
...
def default_prompt(
    *,
    ...
    if scenario == "address":
        prompt = (
            "I'm reviewing a Lucas County, Ohio property. Can you look up the "
            f"county auditor record for {address} and return the assessment record?"
        )
    elif scenario == "owner":
        prompt = (
            "I'm checking Lucas County, Ohio auditor records for an owner. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ask Ollama a natural Lucas County auditor question and log whether "
    ...
    parser.add_argument(
        "--parcel-id",
        default="10-12345",
        help="Lucas County parcel ID for the parcel scenario",
    )
    parser.add_argument(
        "--address",
        default="100 EXAMPLE AVE",
        help="Lucas County property address for the address scenario",
    )
    parser.add_argument(
        "--owner-name",
        default="DOE JANE A",
...
def main() -> None:
    ...
    final_text = asyncio.run(
        run_tool_trigger_sample(
            ...
            logger=LOGGER,
            server_params_factory=lucas_server_params,
        )
    )
    print(final_text)
